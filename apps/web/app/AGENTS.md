...
- Limit analytics to one personless, cookieless `$pageview` on `/`, `/download`, `/alternatives`, and the exact comparison routes accepted by `isHraPublicComparisonPath`. Accept exact Production `https://hra-weld.vercel.app` as site ID `hra-v0` and the temporary exact `https://hra.sh` compatibility alias as site ID `hra`. Use only the slim no-external SDK entry, retain the bounded provider user-agent field required for cookieless visitor hashing, redact query, fragment, referrer, identity, account, task, command, and provider context, and keep every other event and route inert.
