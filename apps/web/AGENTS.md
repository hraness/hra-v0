...
- Send only anonymous, cookieless `$pageview` events from exact public routes on the canonical archive origin and its temporary exact `hra.sh` compatibility alias. Attribute the archive to site ID `hra-v0` and the alias to site ID `hra`. Keep the public PostHog token Production-only, preserve the bounded provider user-agent field required for cookieless visitor hashing, and leave Preview and every authenticated, account, task, and control-plane route without analytics ingestion capability.
