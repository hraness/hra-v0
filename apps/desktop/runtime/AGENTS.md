...
- `release-download-contract.ts`, `release-provenance.ts`, and `release-download.json` at the repository root – immutable candidate/publication evidence, the reviewed archive-repository coordinate migration, and hermetic canonical Git provenance for downloadable releases.
