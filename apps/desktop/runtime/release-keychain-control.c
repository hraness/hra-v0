...
typedef CFDataRef (^SecCodeRemoteSignHandler)(
  CFDataRef,
...
typedef OSStatus (*SecCodeSignerRemoteAddSignatureFunction)(
  SecCodeSignerRemoteRef,
  SecStaticCodeRef,
  SecCSFlags,
  SecCodeRemoteSignHandler,
  CFErrorRef *
);
...
static bool remote_sign(
  ...
  SecCodeSignerRemoteAddSignatureFunction remote_add =
    (SecCodeSignerRemoteAddSignatureFunction)dlsym(
      RTLD_DEFAULT,
      "SecCodeSignerRemoteAddSignature"
    );
  ok = ok
    && identifier_key != NULL
    && digest_key != NULL
